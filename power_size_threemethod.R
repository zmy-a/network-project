...
#function of the test statistic for the other three method
...
# find the size and power of the three test
...
#Combine the results of our method and the other three methods
...
# function of the three test for network data
...
#function of the three test to get p-value of real data
p_value_three<-function(k){
  p_value_1979=rep(0,k)
  p_value_general=rep(0,k)
  p_value_weighted=rep(0,k)
  TEST<-TEST_mst_netdata(k,rest_park,rest_park_con)
  #TEST<-TEST_mst_netdata(k,local_park,local_park_con)
  for (j in 1:k) {
...
#
p_three<-p_value_three(10)
p_three_local<-p_value_three(10)
p_three_rest<-p_value_three(10)

#function of our method to find p-value
...
times=5000
#Bring up the obvious differences in the original data
local_park<-am_array[,100:150,100:150]
local_park_con<-am_con_array[,100:150,100:150]

rest_park<-am_array[,-c(100:150),-c(100:150)]
rest_park_con<-am_con_array[,-c(100:150),-c(100:150)]

#p-value of the different part of pd data set
p_value_f<-function(k,sta){
  rank1=rep(0,k)
  p_value_rank=rep(0,k)
  for (j in 1:k) {
    rank1[j]<-T_sta_rank(j,rest_park,rest_park_con)
    #rank1[j]<-T_sta_rank(j,local_park,local_park_con)
...
pvalue_park_local<-p_value_f(10,sta)
pvalue_park_rest<-p_value_f(10,sta)


#Combine the results of our method and the other three methods
pvalue<-matrix(0,10,4)
pvalue[,1:3]<-p_three
pvalue[,4]<-pvalue_park
...
ppp<-matrix(0,10,3)
ppp[,1]<-p_three_local[,1]
ppp[,2]<-p_three[,1]
ppp[,3]<-p_three_rest[,1]
colnames(ppp)<-c("local","all","rest")
plot_ppp<-melt(ppp)
plot_ppp$k<-plot_ppp$Var1
ggplot(plot_ppp, aes(x=k, y=value)) + geom_line(aes(color=Var2))+geom_point(size=3,aes(shape=Var2,colour=Var2))+labs(y="p-value",fill="shapes")+ggtitle("p-value of different part of pd data")+ theme(plot.title = element_text(hjust = 0.5,size=23))+theme(legend.position=c(0.94,0.85))+theme(legend.title =element_blank())+theme(legend.key.size = unit(30, "pt"))+ theme(legend.text=element_text(size=20))+ theme(axis.title=element_text(size=20),axis.title.x =element_text(size=20), axis.title.y=element_text(size=20),axis.text.y=element_text(size=20),axis.text.x=element_text(size=20))
